...
struct Shader {
    var baseName: String
    var highPerfName: String {
        get {
            return "\(baseName)_highperf"
        }
    }
    var lowPerfName: String {
        get {
            return "\(baseName)_lowperf"
        }
    }
}
...
final class ShaderProgram {
    ...
    // load the best shader availlable for the device
    func load(shader: Shader) -> Bool {
        
        // metal support is our base to enable HQ shaders.
        if MTLCreateSystemDefaultDevice() != nil {
            if load(name: shader.highPerfName) == false {
                if load(name: shader.baseName) == false {
                    if load(name: shader.lowPerfName) == false {
                        return false
                    }
                }
            }
